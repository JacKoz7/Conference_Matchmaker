def print_outcome(algorithm, solution) -> None:
    for i in range(len(algorithm.give_desired_attributes(solution))):
        print(f"Participant {i+1} desired attributes: {algorithm.give_desired_attributes(solution)[i]}")
    print()

    for j in range(len(algorithm.give_attributes(solution))):
        print(f"Attributes of participants matched for Participant {j+1}: {algorithm.give_attributes(solution)[j]}")
    print()
    ...
    for i, p in enumerate(solution.values()):
        ...
        print(f"Participant number {i + 1} matched with participants with IDs {participants_ids}")